...
func BookCreate1() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			...
			logger.Logger.Error("book param bind json error", zap.String("error", err.Error())) //使用zap包的Error()方法
			//下面为几种调用zap打印日志方式
		...
		if err != nil {
			...
			logger.Logger.Info("保存数据成功", zap.String("msg", "Success")) //保存数据成功信息也在日志中打印出来
